...
# Script to build and run the Nager v104 MCP Server locally in Docker
...
IMAGE_NAME="nager-v104-mcp-server"
CONTAINER_NAME="nager-v104-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nager v104 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nager v104 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Nager v104 MCP Server is running!${NC}"
echo
